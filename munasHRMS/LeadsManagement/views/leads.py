...
from munasHRMS.generic.services.utils import constants, decorators, common_utils, response_codes, response_utils
from munasHRMS.config import config

leads_bp = Blueprint("leads_bp", __name__)


@leads_bp.route("/create", methods=["GET"])
@decorators.is_authenticated
@decorators.keys_validator()
def leads_create_view(data):
    res = {}
    res['projects'] = ProjectController.get_projects()
...
@leads_bp.route("/SubZero", methods=["GET"])
@decorators.is_authenticated
def sub_zero():
    if request.method == "GET":
        data = request.args
        res = LeadsController.subzero_controller(data=data)
        return render_template("viewleads.html", **res)
...
@leads_bp.route("/update", methods=["GET","POST"])
@decorators.is_authenticated
@decorators.keys_validator(
    [],
