...
class LeadsController(Controller):
    ...
    def create_controller(cls, data):
        ...
        if already_exists:
            user_leads=[]
            for obj in already_exists:
                user_leads.append({"organization":str(obj[constants.LEAD__ASSIGNED_TO].fetch().organization.id),"id":str(obj[constants.LEAD__ASSIGNED_TO].fetch().id)})
            current_user_org = str(current_user[constants.USER__ORGANIZATION].id)
            if current_user_org == user_leads[0]["organization"]:
                if str(current_user.id) == user_leads[0]["id"]:
                    commit_record = True
                else:                    
                    return response_utils.get_response_object(
    ...
    def read_controller(cls, data):
        ...
        filter_data = {**data}
        filter_fields = {
            "project":ProjectController.get_projects(),
        ...
        if data.get(constants.LEAD__LEVEL):
            if data[constants.LEAD__LEVEL] == 'SubZero':
                del filter['assigned_to__in']
            if data[constants.LEAD__LEVEL] == 'AtomBomb':
                del filter['assigned_to__in']
        ...
        if data.get('id'):
            filter[constants.ID] = data.get('id')
    ...
    @classmethod
    def subzero_controller(cls, data):
        filter = {}
        filter_data = {**data}
        filter_fields = {
        ...
        queryset = cls.db_read_records(read_filter={constants.LEAD__LEVEL:"SubZero"}).order_by('-id').paginate(page=page, per_page=per_page)
        if data.get('edit'):
            lead_dataset = [obj.display_edit() for obj in queryset.items]
        else:
            lead_dataset = [obj.display_min() for obj in queryset.items]
    ...
    def update_controller(cls, data):
        ...
        return response_utils.get_response_object(
            response_code=response_codes.CODE_SUCCESS,
            response_message=response_codes.MESSAGE_SUCCESS,
            response_data={'data':[obj.display()]},
        )
