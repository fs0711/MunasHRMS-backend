...
from munasHRMS.LeadsManagement.models.FollowUp import FollowUp
...
class ReportsController(Controller):
    ...
    def read_kpi(cls, data, data2=None):
        ...
        if data.get(constants.DATE_FROM):
            ...
            filter[constants.CREATED_ON +
                   "__lte"] = common_utils.convert_to_epoch1000(dateto, format=config.FILTER_DATETIME_FORMAT)
            filter_call[constants.CALL_RECORD__DATE +
                   "__gte"] = common_utils.convert_to_epoch1000(datefrom, format=config.FILTER_DATETIME_FORMAT)
            filter_call[constants.CALL_RECORD__DATE +
                   "__lte"] = common_utils.convert_to_epoch1000(dateto, format=config.FILTER_DATETIME_FORMAT)
        ...
        kpi_dataset = {str(user[constants.ID]): {
            ...
            "V_calls": {"INCOMING":{}, "OUTGOING":{}, "MISSED":{}, "Rejected":{}, "Unknown":{}, "Incoming":{}, "Outgoing":{}, "Missed":{},},
            "TLW": 0,
            "lead_count": 0,
            "transfered": 0
        ...
        queryset = CallRecordController.db_read_records(read_filter={**filter_call}).aggregate(
            pipeline.KPI_REPORT_CALL_RECORDS)
        for call in queryset:
            kpi_dataset[str(call["_id"]["created_by"])]["V_calls"][call["_id"]["call_type"]][call["_id"]["duration"]] = {"count":call["count"], "duration":common_utils.format_time(call["total_time"]), "q_calls":call["q_calls"]}
